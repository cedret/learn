...
ConfigNETCARD()
{
    ...
    NetCard=$(ip -4 route show | grep "^default" | awk 'NR == 1 {print $5}')
    if [ -n "$NetCard" ]
    then
        echo -e "La carte reseau active est :[${vert}$NetCard${blanc}]"
        echo -e "Confirmez vous cette carte reseau O/n ?: \t\c"
        read reponse
    else
        reponse="non"
    fi
    ...
    case "$reponse" in
        "" | [oO]   )   return;;
                *   )   IFS=" "
                        while true
                        do
                            TabNetCard=($(nmcli  -f DEVICE device | tr -d "\n"))
                            NbNetCard=${#TabNetCard[*]}
 
                            echo " Le nombre de carte est : $NbNetCard"
                            echo -e "${vert} LES CARTES SONT : ${TabNetCard[*]}${blanc}"
                            nmcli -f DEVICE device
                            echo -e "Veuillez saisir le nom de votre carte reseau:\t\c"
                            read NetCard  
                            trouve=0
                            i=1
                            #On va chercher si la carte saisie est dans le tableau des cartes
                            while [ $i -lt $NbNetCard -a $trouve -eq 0 ]
                            do
                                    lacarte=${TabNetCard[$i]}
                                    if [ "$NetCard" = "$lacarte" ]
                                    then
                                        trouve=1
                                    fi
                            ...
                            if [ $trouve -eq 1 ]
                            then
                                echo -e "Votre carte est : ${vert}$NetCard${blanc}"
                                break
                            fi
                        done
                        ;;
...
ajoulogi()
{
    ...
    if [ $erreurs -eq 0 ]
    then
        echo -e "${vert} Le reseau est ok${blanc}"
    else
...
echo " start "
# si EUID = 0 afficher en vert le script va s'executer et bascule blanc
# sinon en reouge, passer root pour executer le script, bascule blanc
# sortir du script
...
echo -e "Entrez 11 configurer une IP en bridge ou continuer"
read reponse
if [ $reponse -eq 11 ]
then
...
echo -e "Entrez 12 si vous voulez installer les compléments logiciels ou continuer"
read reponse
if [ $reponse -eq 12 ]
then
    ajoulogi
...
ip a
echo -e "Entrez 13 pour configurer une ip pour serveur PXE ou continuer"
read reponse
if [ $reponse -eq 13 ]
then
...
echo -e "***** Valeurs carte ******"
echo -e "${orange}IPa=$IPa IPb=$IPb IPc=$IPc IPd=$IPd${blanc}"
DNS=$DNSa.$DNSb.$DNSc.$DNSd
echo -e "${orange}DNSa=$DNSa DNSb=$DNSb DNSc=$DNSc DNSd=$DNSd${blanc}"
echo "$IPa.$IPb.$IPc.$IPd-$MASK-$GTW-$DNS"

echo -e "Entrez 14 si vous voulez activer les services dhcp et tftp ou continuer"
read reponse
if [ $reponse -eq 14 ]
then
...
echo -e "Entrez 15 si vous voulez connecter DVD avec fichier ISO ou continuer"
read reponse
if [ $reponse -eq 15 ]
then
# echo -e "${rouge} Vérifier que le lecteur est connecté avec un fichier ISO dedans${blanc}"
...
echo -e "Entrez 16 pour activer NFS ou continuer"
read reponse
if [ $reponse -eq 16 ]
then
...
echo -e "Entrez 17 pour remplacer ks.cfg ou continuer"
read reponse
...
echo -e "Entrez 18 pour vérifier les contenus modifiés"
read reponse
if [ $reponse -eq 18 ]
then
    ls -al /var/lib/tftpboot/pxelinux.cfg
    ls -al /etc/exports
    ls -al /SrcAlma
    ls -al /reponses/ks.cfg
    cat /etc/dhcp/dhcpd.conf
